...
const DISCOVERY_IDEAS = [
  { id: 'bf', label: 'Sunrise Breakfast', age: '6–12 months', type: 'Breakfast', icon: '🍳' },
  { id: 'ff', label: 'NHS Finger Foods', age: '6–12 months', type: 'Finger Food', icon: '🥕' },
  { id: 'ld', label: 'Energy Lunch', age: '12–24 months', type: 'Lunch/Dinner', icon: '🍲' },
  { id: 'sp', label: 'Mild Spice Curry', age: '2-3 years', type: 'Lunch/Dinner', icon: '🍛' },
];
...
const MEAL_TYPES = [
  ...
  { id: 'Snack/Pudding', label: 'Snack/Pudding' },
];
...
export default function App() {
  ...
  const applyDiscoveryIdea = (idea: typeof DISCOVERY_IDEAS[0]) => {
    setAgeGroup(idea.age);
    setSelectedMealType(idea.type);
    // Add some common pairing ingredients for the vibe
    if (idea.type === 'Breakfast') setSelectedIngredients(['Eggs', 'Oats']);
    if (idea.type === 'Finger Food') setSelectedIngredients(['Banana', 'Bread']);
    if (idea.type === 'Lunch/Dinner') setSelectedIngredients(['Chicken', 'Broccoli']);
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateRecipe(
        `${ageGroup} (${selectedMealType} style)`, 
        allIngredients, 
    ...
    <div className="min-h-screen bg-brand-cream/50">
      ...
      <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)]">
        ...
        <aside className="w-full lg:w-1/3 border-r border-stone-100 bg-white p-6 space-y-10 overflow-y-auto">
          ...
          {/* Section: Meal Type */}
          <div className="space-y-4">
            <label className="text-[10px] font-display font-black uppercase tracking-[0.2em] text-stone-400">Meal Style</label>
            <div className="grid grid-cols-3 gap-2">
              {MEAL_TYPES.map(type => (
                <button
                  key={type.id}
              ...
              >
                <div className="p-8 bg-white border border-stone-100 rounded-3xl shadow-xl shadow-stone-200/50 flex flex-col items-center gap-6">
                  ...
                  <div className="pt-4 space-y-3">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-300">Or try an NHS favorite:</span>
                    <div className="flex flex-wrap justify-center gap-2 max-w-md">
                      {DISCOVERY_IDEAS.map(idea => (
                        <button
                          key={idea.id}
                          onClick={() => applyDiscoveryIdea(idea)}
                          className="px-4 py-2 bg-stone-50 border border-stone-100 rounded-full text-[10px] font-bold text-stone-500 hover:border-brand-sage hover:bg-brand-cream transition-all flex items-center gap-2"
                        >
